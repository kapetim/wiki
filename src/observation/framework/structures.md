...
flowchart TD
 born[Born]
 branch{First placement}
 family[Family raises you]
 orphan[Orphanage]
 adopt[Adoption]
 mgr[Manager / guardian]
 early[Early stage — dependent]
 grad{Graduate from dependency?}
 society[Join society — own run]
 bank[Bank / accounts]
 work[Work — company or farm etc.]
 live[Daily loop — eat sleep move fun]
 sick{Disease / injury?}
 hospital[Hospital + medication]
 grave[Graveyard]
 born --> branch
 branch --> family --> mgr --> early
 branch --> orphan --> early
 branch --> adopt --> mgr --> early
 early --> grad
 grad -->|yes — expected| society
 grad -->|stuck rare| early
 society --> bank
 society --> work
 society --> live
 live --> sick
 sick -->|recover| live
 sick -->|not recover| grave
 live -->|age accident choice| grave
 work --> live
 bank --> live
```
...
flowchart LR
 ash[Heaven — Ash path]
 rocket[Hell — Rocket path]
 mon[Pokémon / species]
 judge[Rules + judge]
 ash -->|catch battle enjoy| mon
 rocket -->|steal exploit taunt| mon
 rocket -->|irritates| ash
 ash -->|police league| judge
 rocket -->|break rules| judge
```
...
```mermaid
flowchart LR
 heaven[Heaven — castle]
 hell[Hell — volcano lab]
 heaven -->|comfort / police| defend[Defense craft]
 hell -->|explosives / plans| attack[Attack craft]
 attack -->|scores| pause[Celebration window]
 pause --> defend
 defend -->|new shield| attack
```
...
```mermaid
flowchart TD
 serve[Hell serves — attack / reveal]
 recv[Heaven receives — shock]
 defend[Heaven crafts defense]
 score[Hell scores — brief win]
 rest[Hell celebrates — pause]
 serve --> recv --> defend --> score --> rest --> serve
```
