...
```mermaid
flowchart LR
  assess[Assess — name risk once]
  post[Post — one margin]
  reconcile[Reconcile — stop or act]
  assess --> post --> reconcile
  reconcile -->|new data| assess
  reconcile -->|gate closed| stop[Ignore — no debit for thinking]
```
