...
```mermaid
flowchart TB
 trade[Same trade — time attention access]
 maskA[Skin A — cannot be bought]
 maskB[Skin B — price and slot length]
 trade --> maskA
 trade --> maskB
 maskA -->|sentiment wrapper| hide[Hide ledger from self]
 maskB -->|explicit menu| show[Show ledger upfront]
```
...
```mermaid
flowchart LR
 script[Public script — can't be bought]
 vector[Convince vector — what still works]
