...
```mermaid
flowchart LR
  subgraph knowledge["Knowledge — fair"]
    blue["Blue −∞"] --- xk["Workable theory"] --- red["Red +∞"]
  end
  subgraph violence["Violence — unfair"]
    green["Green −∞"] --- xv["Judicious force"] --- purple["Purple +∞"]
  end
  subgraph purpose["Purpose — what vs why"]
    pink["Pink −∞"] --- xp["Meaningful routine"] --- yellow["Yellow +∞"]
  end
  subgraph memory["Memory — ledgers"]
    white["White −∞"] --- xm["Accurate ledger"] --- black["Black +∞"]
  end
  subgraph belief["Belief — fiction vs doubt"]
    orange["Orange −∞"] --- xb["Earned trust"] --- gray["Gray +∞"]
  end
```
